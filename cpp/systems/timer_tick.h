...
inline stagehand::Registry register_enemy_timer_tick_system([](flecs::world &world) {
    world
        .system<ProjectileHitTimeout, ShockwaveHitTimeout, DeathTimer, HitReactionTimer, HFlipTimer, VFlipTimer, const EnemyTakeDamageSettings>(
            "Enemy Timer Tick")
    ...
    world.system<PlayerDamageCooldown, const PlayerTakeDamageSettings>("Player Damage Timer Tick").kind(flecs::PreUpdate).run([](flecs::iter &it) {
        while (it.next()) {
            flecs::field<PlayerDamageCooldown> player_damage_cooldown = it.field<PlayerDamageCooldown>(0);                     // singleton
            flecs::field<const PlayerTakeDamageSettings> player_damage_settings = it.field<const PlayerTakeDamageSettings>(1); // singleton
